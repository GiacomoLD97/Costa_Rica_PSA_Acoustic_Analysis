...
df %>%
  # filter(freq_category %in% c('3-4 kHz', '4-5 kHz', '5-6 kHz', '6-7 kHz', '7-8 kHz', '8-9 kHz')) %>%
  filter(freq_category == '5-6 kHz') %>% 
  mutate(nearest_10 = round_any(Minute, 10, round)) %>% 
  filter(nearest_10 >= 270 & nearest_10 < 370) %>%
  mutate(tod = as.POSIXct("2023-12-13 00:00:00") + minutes(nearest_10)) %>% 
  ggplot(aes(x = sum_PMN, fill = Type, y = as.factor(tod))) + 
  geom_density_ridges(rel_min_height = 0.005, alpha = 0.4) +
  scale_fill_manual(values = c("#4477AA", "#CCBB44", "#EE6677", "#228833")) +
  coord_flip() +
  theme_minimal() +
  ylab("Time of Day") +
  xlab("Power-minus-Noise") 

ggsave('/Users/johanvandenhoogen/ETH/Projects/costa_rica/figures/figure3_ridges.pdf')
